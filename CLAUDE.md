...
**Прод:** VPS в РФ (185.139.70.35), nginx + pm2. Фронт собирается локально и заливается как `client/dist` (см. `tasks/` и память деплоя). 152-ФЗ: политика на `/privacy`, чекбокс согласия в форме, cookie-баннер.
