# AGENTS.md — Best Practice AI: спецификация сайта (актуальное состояние)

> Единый файл для всех ИИ-агентов (Claude Code, Codex, Cursor, Copilot, Gemini CLI и др.). `CLAUDE.md` в корне лишь подключает этот файл — править нужно здесь.
...
> Перед правками nginx, `index.html` или переносом хостинга перечитай раздел 17 «Доступность из РФ».
...
- **Цифры** — тёмная плашка с 4 счётчиками (`AnimatedCounter`): **2** федеральные премии · **300+** часов обучения ИИ · **9,3 / 10** средняя оценка тренингов участниками (`AnimatedCounter decimals={1}`, «/ 10» мельче) · **70%** экономия на контенте. На ≤768px — 2×2.
...
Инцидент 2026-09-05: сайт «не открывался» с iPhone из сетей МТС и домашнего Wi-Fi без VPN, с VPN работал. Сервер, DNS, сертификат и реестр РКН были ни при чём. tcpdump на сервере показал: из мобильной сети до сервера доходят только первые 2–3 TLS-соединения с одного адреса, у остальных по пути молча теряется второй сегмент ClientHello (у iOS 26 / Chrome ClientHello длиннее одного пакета из-за ML-KEM). Safari по HTTP/1.1 открывал 6+ соединений на страницу — большинство висело, экран оставался чёрным.

**Правила:**
1. **HTTP/2 в nginx обязателен** (`listen 443 ssl http2;` в `/etc/nginx/sites-enabled/bestpracticeai`). Все запросы страницы идут в одном TLS-соединении, которое проходит. Не отключать и не заменять на HTTP/1.1 при любых переносах конфига.
2. **Никаких внешних render-blocking ресурсов в `<head>`** (stylesheet/скрипт с чужого домена, особенно Google). Шрифты только локальные (§5). Сторонние сервисы (Kinescope, Метрика) — асинхронно и не в критическом пути.
